...
PACKAGES1="https://marcelotduarte.github.io/packages/cx_Freeze"
PACKAGES2="https://marcelotduarte.github.io/packages/cx_Logging"
WHEELHOUSE="${TOP_DIR}/wheelhouse"
export PIP_FIND_LINKS="$WHEELHOUSE $PACKAGES1 $PACKAGES2"
...
if [ -e Pipfile ] ; then
    if ! pipenv run python -c "${TEST_CXFREEZE}" 2>/dev/null; then
        if [ -d $WHEELHOUSE ] ; then
            echo "::endgroup::"
            echo "::group::Install cx-freeze from packages/wheelhouse"
    ...
    if ! python -c "${TEST_CXFREEZE}" 2>/dev/null; then
        echo "::endgroup::"
        echo "::group::Install cx-freeze from packages/wheelhouse"
        if ! pip install --no-deps --no-index cx_Freeze ; then
            echo "::endgroup::"
            echo "::group::Install cx-freeze from directory of the project"
            pushd $TOP_DIR
            pip install -e . --no-deps --ignore-installed --no-cache-dir -v
